...
class DifferenceInDifferences:
    ...
    def estimate_did(
        ...
        if control_vars:
            # Add control variables
            available_controls = [
                var for var in control_vars if var in self.panel_data.columns
            ]
    ...
    def parallel_trends_test(
        self, pre_periods: list[int] | None = None, outcome_col: str = "outcome"
    ) -> dict[str, Any]:
        """
        ...
        for outcome in [
            ...
            if outcome in baseline_data.columns:
                ...
                corr, p_val = stats.pearsonr(
                    subset["used_smart_assistant"], subset[outcome]
                )
    ...
    def estimate_heterogeneous_effects(
        self, subgroup_var: str, outcome_col: str = "outcome"
    ) -> dict[str, Any]:
        """
        ...
        return {"subgroup_variable": subgroup_var, "results": subgroup_results}
    ...
    def plot_parallel_trends(
        ...
        treated_2023 = self.data[self.data["used_smart_assistant"] == 1][
            outcome_2023_col
        ].mean()
        treated_2024 = self.data[self.data["used_smart_assistant"] == 1][
            outcome_2024_col
        ].mean()
        control_2023 = self.data[self.data["used_smart_assistant"] == 0][
            outcome_2023_col
        ].mean()
        control_2024 = self.data[self.data["used_smart_assistant"] == 0][
            outcome_2024_col
        ].mean()
        ...
        ax.plot(
            ...
            linewidth=2,
            markersize=8,
            label="Treated (Used Smart Assistant)",
            color="blue",
        )
        ax.plot(
            years,
            ...
            linewidth=2,
            markersize=8,
            label="Control (No Smart Assistant)",
            color="red",
        )
    ...
    def plot_subgroup_effects(
        self, subgroup_results: dict[str, Any], figsize: tuple[int, int] = (12, 6)
    ) -> plt.Figure:
        """
        ...
        ax.set_title(
            f"Heterogeneous Treatment Effects by {subgroup_results['subgroup_variable']}"
        )
        ax.grid(True, alpha=0.3)

        # Add value labels
        for i, (est, _sg) in enumerate(zip(estimates, subgroups, strict=False)):
            ax.text(
                est + 0.01, i, f"{est:.3f}", verticalalignment="center", fontsize=10
            )
