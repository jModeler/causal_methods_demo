...
def load_config(config_path: str = "config/simulation_config.yaml") -> dict:
    ...
    if "simulation_config.yaml" not in config_path:
        base_config_path = os.path.join(
            os.path.dirname(config_path), "simulation_config.yaml"
        )
...
class TaxSoftwareDataSimulator:
    ...
    def __init__(
        self, n_users: int = None, config_path: str = "config/simulation_config.yaml"
    ):
        """
        ...
        self.n_users = (
            ...
            else self.config["simulation"]["default_n_users"]
        )
    ...
    def generate_user_demographics(self) -> pd.DataFrame:
        ...
        for i in range(self.n_users):
            ...
            if region == "West":
                ...
                tech_base += region_adj[
                    "midwest_south_penalty"
                ]  # Note: this is negative

            tech_savviness = max(
                tech_config["min_score"],
                min(
                    tech_config["max_score"],
    ...
    def generate_2023_baseline_data(self, users_df: pd.DataFrame) -> pd.DataFrame:
        ...
        for _, user in users_df.iterrows():
            ...
            filed_2023 = np.random.random() < self._get_filing_probability(
                user, filing_config
            )
            ...
            if filed_2023:
                ...
                if user["user_type"] == "returning":
                    base_time += time_config[
                        "returning_user_reduction"
                    ]  # negative value
    ...
    def generate_treatment_assignment(
        self, users_df: pd.DataFrame, baseline_df: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        for _, user in users_df.iterrows():
            baseline_user = baseline_df[baseline_df["user_id"] == user["user_id"]].iloc[
                0
            ]
            ...
            # Apply bounds
            use_prob = max(
                treatment_config["min_probability"],
    ...
    def generate_2024_outcomes(
        ...
        for _, user in users_df.iterrows():
            ...
            treatment_user = treatment_df[
                treatment_df["user_id"] == user["user_id"]
            ].iloc[0]
            ...
            if filing_config["apply_demographic_effects"]:
                ...
                base_filing_prob = self._get_filing_probability(
                    user, base_filing_config
                )
            else:
            ...
            filing_prob_2024 = min(
                filing_config["max_probability"], base_filing_prob + treatment_effect
            )
            ...
            if filed_2024:
                ...
                base_time = (
                    baseline_user["time_to_complete_2023"]
                    if baseline_user["time_to_complete_2023"] > 0
                    else 120
                )
                ...
                if baseline_user["support_tickets_2023"] > 0:
                    base_satisfaction += satisfaction_config[
                        "support_history_penalty"
                    ]  # negative value

                satisfaction_2024 = max(
                    satisfaction_config["min_score"],
                    min(
                        satisfaction_config["max_score"],
    ...
    def _get_filing_probability(self, user: pd.Series, filing_config: dict) -> float:
        ...
        # Age effects
        if (
            filing_config["prime_age_min"]
            <= user["age"]
            <= filing_config["prime_age_max"]
        ):
        ...
        if output_config["include_derived_features"]:
            for feature in output_config["derived_features"]:
                if feature == "time_improvement":
                    complete_df["time_improvement"] = (
                        complete_df["time_to_complete_2023"]
                        - complete_df["time_to_complete_2024"]
                    )
                elif feature == "session_improvement":
                    complete_df["session_improvement"] = (
                        complete_df["sessions_2023"] - complete_df["sessions_2024"]
                    )
...
def generate_and_save_data(
    ...
    print(
        f"Naive treatment effect: {(treated['filed_2024'].mean() - control['filed_2024'].mean()):.1%}"
    )
